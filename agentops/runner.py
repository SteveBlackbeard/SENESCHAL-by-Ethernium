...
def run_request(
    *,
    ...
    if not plan.should_call:
        ...
        return RunResult(plan=plan, response=response, prompt_tokens=prompt_tokens, ledger_path=str(ledger_path) if ledger_path else None)
    ...
    if response.ok:
        ...
        mark_provider_state(profile.id, status="fail", reason=response.error, path=state_file)
    ...
    if ledger_path is not None:
        append_entry(
            ...
            new_entry(
                ...
                retries=0,
                outcome="ok" if response.ok else response.error,
                reduced="cost",
            ),
        )
    return RunResult(plan=plan, response=response, prompt_tokens=prompt_tokens, ledger_path=str(ledger_path) if ledger_path else None)
